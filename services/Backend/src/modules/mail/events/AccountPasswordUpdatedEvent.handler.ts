...
@Injectable()
export class AccountPasswordUpdatedEventHandler<T> implements IInboxEventHandler {
    constructor(
    ...
    public async handle(event: IntegrationEvent): Promise<void> {
        ...
        try {
            await this.mailer.send(payload.email, this.emailFactory.createPasswordUpdatedEmail());
        } catch (e) {
