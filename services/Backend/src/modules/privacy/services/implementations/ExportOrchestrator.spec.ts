...
import { NoOpExportEventsPublisher } from "@/modules/privacy/services/implementations/NoOpExportEventsPublisher";
...
describe("ExportOrchestrator", () => {
    ...
    beforeAll(async () => {
        ...
        dataExportRepository = app.get(getRepositoryToken(DataExportEntity, PRIVACY_MODULE_DATA_SOURCE));
        attachmentRepository = app.get(getRepositoryToken(ExportAttachmentManifestEntity, PRIVACY_MODULE_DATA_SOURCE));
        tenantRepository = app.get(getRepositoryToken(TenantEntity, PRIVACY_MODULE_DATA_SOURCE));
    });
