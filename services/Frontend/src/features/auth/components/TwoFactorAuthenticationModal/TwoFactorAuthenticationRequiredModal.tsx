...
type TwoFactorAuthenticationRequiredModalProps = {
...
export const TwoFactorAuthenticationRequiredModal = ({
    scopesToActivate,
    method,
    onMethodSelected,
}: TwoFactorAuthenticationRequiredModalProps) => {
    const t = useTranslate();
    const [code, setCode] = useState<string>("");

    const { data: integrations } = useGetActive2FAIntegrations();
    const availableMethods = (integrations ?? []).map(({ method }) => method);
