...
export const useUpdateEntryIsFeatured = () => {
    const { revert, update, invalidate } = useQueryCache();

    return useMutation({
        mutationFn: EntriesService.updateIsFeatured,
        onMutate: async ({ entryId, isFeatured }) => {
            return await update<InfiniteData<PageDto<Entry>>>(queryKey, ({ pages, pageParams }) => {
                const newPages =
                    pages.map((page) => {
                        return {
                            ...page,
                            data: page.data.map((entry) => (entry.id === entryId ? { ...entry, isFeatured } : entry)),
                        };
                    }) ?? [];
