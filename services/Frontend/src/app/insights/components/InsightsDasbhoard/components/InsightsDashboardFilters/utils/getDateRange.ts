...
export const getDateRange = (preset: DateRangePreset): ISODateStringRange => {
    ...
    switch (preset) {
        case DateRangePreset.PAST_YEAR:
            return { from: format(sameDayLastYear), to: format(now) };
        case DateRangePreset.THIS_YEAR:
            return { from: format(now.startOf("year")), to: format(now) };
        ...
        case DateRangePreset.PAST_MONTH:
            return { from: format(sameDayLastMonth), to: format(now) };
        case DateRangePreset.THIS_MONTH:
            return { from: format(now.startOf("month")), to: format(now) };
