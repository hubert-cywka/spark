resource "kubernetes_deployment" "journal-service" {
    metadata {
...
resource "kubernetes_service" "journal-service" {
    metadata {
        name      = "journal-service"
        namespace = kubernetes_namespace.codename.metadata[0].name
    }
    ...
    spec {
        selector = {
            app = kubernetes_deployment.journal-service.spec[0].template[0].metadata[0].labels.app
        }
