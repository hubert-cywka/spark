resource "kubernetes_deployment" "identity-service" {
    metadata {
    ...
    spec {
        ...
        template {
            ...
            spec {
                container {
                    name  = "identity-service"
                    image = "hejs22/codename-identity-service:latest"
...
resource "kubernetes_service" "identity-service" {
    metadata {
        name      = "identity-service"
        namespace = kubernetes_namespace.codename.metadata[0].name
    }
    ...
    spec {
        selector = {
            app = kubernetes_deployment.identity-service.spec[0].template[0].metadata[0].labels.app
        }
