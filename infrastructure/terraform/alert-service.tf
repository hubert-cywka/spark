resource "kubernetes_deployment" "alert-service" {
    metadata {
        name      = "alert-service"
    ...
    spec {
        ...
        template {
            ...
            spec {
                container {
                    name  = "alert-service"
                    image = "hejs22/codename-alert-service:latest"
...
resource "kubernetes_service" "alert-service" {
    metadata {
        name      = "alert-service"
        namespace = kubernetes_namespace.codename.metadata[0].name
    }
    ...
    spec {
        selector = {
            app = kubernetes_deployment.alert-service.spec[0].template[0].metadata[0].labels.app
        }
